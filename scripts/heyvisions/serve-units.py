...
def main() -> int:
    ...
    ap.add_argument("--root", default="~/projects/website/example/src/course-content/units")
    ...
    if not root.is_dir():
        raise SystemExit(f"单元目录不存在：{root}")

    handler = partial(CORSHandler, directory=str(root))
    print(f"服务 {root} 于 http://{args.host}:{args.port}/<unitKey>.md")
